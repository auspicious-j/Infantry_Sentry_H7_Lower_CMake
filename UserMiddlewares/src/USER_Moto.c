...
{
	motor->totalAngle = motor->nowAngle;
	if (motor->totalAngle > 360 / 2.0f)
		motor->totalAngle -= 360;
	motor->lastAngle = motor->nowAngle;
	motor->targetTurnAngle = 0;
}
void Motor_CalcAngle_J4310(DM_motor_t *motor)
{
	float dAngle = 0;
	if (motor->nowAngle - motor->lastAngle < -180)
		dAngle = motor->nowAngle + (360 - motor->lastAngle);
	else if (motor->nowAngle - motor->lastAngle > 180)
		dAngle = -motor->lastAngle - (360 - motor->nowAngle);
	else
		dAngle = motor->nowAngle - motor->lastAngle;
